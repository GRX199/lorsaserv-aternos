...
function startHealthServer(port = 3000, getStatusCallback, config, getStatusManagerCallback) {
  const server = http.createServer((req, res) => {
    ...
    if (pathname === '/world-map' || pathname === '/worldmap' || pathname.startsWith('/world-map/')) {
      const fs = require('node:fs');
      const path = require('node:path');
      const worldMapDir = path.join(__dirname, '..', 'public', 'world_map');

      let relPath = pathname.replace(/^\/(?:world-map|worldmap)\/?/, '');
      if (!relPath || relPath === '/') {
        relPath = 'index.html';
      }

      // Khusus custom.markers.js: Sajikan koordinat live pemain secara dinamis!
      if (relPath === 'custom.markers.js') {
        ...
        const js = `// Live player markers generated by lorsaserv\nvar UnminedCustomMarkers = ${JSON.stringify(markers, null, 2)};\n`;
        res.writeHead(200, {
