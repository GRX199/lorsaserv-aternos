  ...
  const server = http.createServer((req, res) => {
    ...
    if (pathname.startsWith('/world-map/')) {
      ...
      if (relPath === 'custom.markers.js') {
        ...
        // Offline ditaruh di bawah agar online player selalu berada di lapisan terdepan
        const markers = [...offlineMarkers, ...onlineMarkers];
    ...
    <style id="unmined-dark-theme">
      ...
      img.ol-tile {
        ...
        if (!html.includes('unmined-zoom-booster')) {
          const zoomScript = `
    <script id="unmined-zoom-booster">
      // Tingkatkan batas zoom-in web viewer agar dapat melihat bangunan lebih dekat
      if (typeof UnminedMapProperties !== 'undefined') {
        if (typeof UnminedMapProperties.maxZoom === 'number') {
          UnminedMapProperties.maxZoom += 3;
        }
      }
      window.addEventListener('DOMContentLoaded', function() {
        setTimeout(function() {
          if (typeof unmined !== 'undefined' && unmined && unmined.map) {
            var v = unmined.map.getView();
            if (v && typeof v.getMaxZoom === 'function') {
              var curMax = v.getMaxZoom();
              if (curMax < 8) {
                v.setMaxZoom(curMax + 3);
              }
            }
          }
        }, 300);
          ...
          html = html.replace(/<\/head>/i, `${zoomScript}\n</head>`);
        }
        res.writeHead(200, {
          'Content-Type': 'text/html; charset=utf-8',
          'Cache-Control': 'no-cache, no-store, must-revalidate'
